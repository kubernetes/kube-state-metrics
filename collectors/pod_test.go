...
func TestPodCollector(t *testing.T) {
	...
	cases := []struct {
	...
	}{
		{
			...
			want: metadata + `
				kube_pod_info{created_by="<none>",host_ip="1.1.1.1",namespace="ns1",pod="pod1",node="node1",pod_ip="1.2.3.4",owner_kind="<none>",owner_name="<none>",controller="<none>"} 1
				kube_pod_info{created_by="<none>",host_ip="1.1.1.1",namespace="ns2",pod="pod2",node="node2",pod_ip="2.3.4.5",owner_kind="ReplicaSet",owner_name="rs-name",controller="true"} 1
				`,
