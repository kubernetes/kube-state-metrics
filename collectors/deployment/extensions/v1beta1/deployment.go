...
func RegisterDeploymentCollector(registry prometheus.Registerer, kubeClient kubernetes.Interface, namespace string) {
	...
	dinf := cache.NewSharedInformer(dlw, &v1beta1.Deployment{}, collectors.ResyncPeriod)
...
func (dc *deploymentCollector) Collect(ch chan<- prometheus.Metric) {
	...
	if err != nil {
		collectors.ScrapeErrorTotalMetric.With(prometheus.Labels{"resource": "deployment"}).Inc()
		glog.Errorf("listing deployments failed: %s", err)
		return
	}

	collectors.ResourcesPerScrapeMetric.With(prometheus.Labels{"resource": "deployment"}).Observe(float64(len(ds)))
	for _, d := range ds {
...
func (dc *deploymentCollector) collectDeployment(ch chan<- prometheus.Metric, d v1beta1.Deployment) {
	...
	labelKeys, labelValues := collectors.KubeLabelsToPrometheusLabels(d.Labels)
	...
	addGauge(descDeploymentSpecPaused, collectors.BoolFloat64(d.Spec.Paused))
