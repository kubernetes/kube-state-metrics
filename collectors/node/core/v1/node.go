...
func RegisterNodeCollector(registry prometheus.Registerer, kubeClient kubernetes.Interface, namespace string) {
	...
	ninf := cache.NewSharedInformer(nlw, &v1.Node{}, collectors.ResyncPeriod)
...
func (nc *nodeCollector) Collect(ch chan<- prometheus.Metric) {
	...
	if err != nil {
		collectors.ScrapeErrorTotalMetric.With(prometheus.Labels{"resource": "node"}).Inc()
		glog.Errorf("listing nodes failed: %s", err)
		return
	}

	collectors.ResourcesPerScrapeMetric.With(prometheus.Labels{"resource": "node"}).Observe(float64(len(nodes.Items)))
...
func (nc *nodeCollector) collectNode(ch chan<- prometheus.Metric, n v1.Node) {
	...
	labelKeys, labelValues := collectors.KubeLabelsToPrometheusLabels(n.Labels)
	addGauge(nodeLabelsDesc(labelKeys), 1, labelValues...)

	addGauge(descNodeSpecUnschedulable, collectors.BoolFloat64(n.Spec.Unschedulable))
	...
	for _, c := range n.Status.Conditions {
		...
		collectors.AddConditionMetrics(ch, descNodeStatusCondition, c.Status, n.Name, string(c.Type))
	}

	// Set current phase to 1, others to 0 if it is set.
	if p := n.Status.Phase; p != "" {
		addGauge(descNodeStatusPhase, collectors.BoolFloat64(p == v1.NodePending), string(v1.NodePending))
		addGauge(descNodeStatusPhase, collectors.BoolFloat64(p == v1.NodeRunning), string(v1.NodeRunning))
		addGauge(descNodeStatusPhase, collectors.BoolFloat64(p == v1.NodeTerminated), string(v1.NodeTerminated))
	}
