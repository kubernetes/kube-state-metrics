...
func RegisterJobCollector(registry prometheus.Registerer, kubeClient kubernetes.Interface, namespace string) {
	...
	jinf := cache.NewSharedInformer(jlw, &v1batch.Job{}, collectors.ResyncPeriod)
...
func (jc *jobCollector) Collect(ch chan<- prometheus.Metric) {
	...
	if err != nil {
		collectors.ScrapeErrorTotalMetric.With(prometheus.Labels{"resource": "job"}).Inc()
		glog.Errorf("listing jobs failed: %s", err)
		return
	}

	collectors.ResourcesPerScrapeMetric.With(prometheus.Labels{"resource": "job"}).Observe(float64(len(jobs)))
	for _, j := range jobs {
		jc.collectJob(ch, j)
	}
...
func (jc *jobCollector) collectJob(ch chan<- prometheus.Metric, j v1batch.Job) {
	...
	labelKeys, labelValues := collectors.KubeLabelsToPrometheusLabels(j.Labels)
	...
	for _, c := range j.Status.Conditions {
		...
		case v1batch.JobComplete:
			collectors.AddConditionMetrics(ch, descJobConditionComplete, c.Status, j.Namespace, j.Name)
		case v1batch.JobFailed:
			collectors.AddConditionMetrics(ch, descJobConditionFailed, c.Status, j.Namespace, j.Name)
		}
	}
}
