...
func main() {
	...
	err = mergeCollectors(collectors, options.collectorsConfig)
	if err != nil {
	...
	err = validateCollectorsAgainstServer(supportedResourceMetaInfo, collectors)
	if err != nil {
...
// validateCollectorsAgainstServer checks whether the specified collectors are validating to the server.
func validateCollectorsAgainstServer(supportedResourceMetaInfo map[string][]resourceMetaInfo, enabledCollectors collectorsMap) error {
			...
			for _, rmi := range rmis {
				if rmi.GroupVersion.String() == groupVersion {
					if !rmi.Verbs.HasAll(requiredVerbs...) {
						return fmt.Errorf("resource %q does not support all these verbs: %v", c, requiredVerbs)
					}
				}
			}
		}
...
func mergeCollectors(enabledCollectors collectorsMap, collectorsConfig collectorsConfigMap) error {
	...
	for c, groupVersion := range enabledCollectors {
		if fns, ok := registeredCollectors[c]; ok {
			if f, exists := fns[groupVersion]; exists {
				...
				glog.Fatalf("Group version %q for collector %q is not registered", groupVersion, c)
			}
		} else {
			glog.Fatalf("Enabled collector %q is not registered", c)
		}
	}

	glog.Infof("Active collectors: %s", enabledCollectors)
}
