...
var (
	...
	descIngressLoadBalancer = prometheus.NewDesc(
		...
		[]string{"namespace", "name", "IP", "hostname"}, nil,
	)
)
...
func RegisterIngressCollector(registry prometheus.Registerer, kubeClient kubernetes.Interface) {
	client := kubeClient.ExtensionsV1beta1().RESTClient()
	rslw := cache.NewListWatchFromClient(client, "ingresses", api.NamespaceAll, nil)
	rsinf := cache.NewSharedInformer(rslw, &v1beta1.Ingress{}, resyncPeriod)

	ingressLister := IngressLister(func() (ingresses []v1beta1.Ingress, err error) {
		for _, c := range rsinf.GetStore().List() {
			ingresses = append(ingresses, *(c.(*v1beta1.Ingress)))
		}
...
type ingressStore interface {
	List() (ingresses []v1beta1.Ingress, err error)
}
