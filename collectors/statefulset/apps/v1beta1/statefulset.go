...
func RegisterStatefulSetCollector(registry prometheus.Registerer, kubeClient kubernetes.Interface, namespace string) {
	...
	dinf := cache.NewSharedInformer(dlw, &v1beta1.StatefulSet{}, collectors.ResyncPeriod)
...
func (sc *statefulSetCollector) Collect(ch chan<- prometheus.Metric) {
	...
	if err != nil {
		collectors.ScrapeErrorTotalMetric.With(prometheus.Labels{"resource": "statefulset"}).Inc()
		glog.Errorf("listing statefulsets failed: %s", err)
		return
	}

	collectors.ResourcesPerScrapeMetric.With(prometheus.Labels{"resource": "statefulset"}).Observe(float64(len(sss)))
	for _, d := range sss {
...
func (dc *statefulSetCollector) collectStatefulSet(ch chan<- prometheus.Metric, statefulSet v1beta1.StatefulSet) {
	...
	labelKeys, labelValues := collectors.KubeLabelsToPrometheusLabels(statefulSet.Labels)
