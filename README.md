...
With the `--server-side-sharding` flag, kube-state-metrics instead asks the kube-apiserver to filter list and watch responses down to the instance's shard using a `shardSelector` (KEP-5866), so each shard only receives, unmarshals, and processes its own subset of objects. This reduces network traffic and resource consumption to roughly 1/n per shard. It requires Kubernetes 1.36 or newer with the `ShardedListAndWatch` feature gate enabled on the kube-apiserver; when the apiserver does not apply the shard selector, kube-state-metrics transparently falls back to the client-side filtering described above. Note that this mode assigns objects to shards by contiguous hash ranges instead of jump hashing, so toggling the flag moves metrics between shards during a rollout. This is experimental.
