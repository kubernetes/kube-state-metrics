...
`,
		...
		},
		{
			...
			Want: `
					# HELP kube_persistentvolume_access_mode The access mode(s) specified by the persistent volume.
	...
	for i, c := range cases {
		c.Func = generator.ComposeMetricGenFuncs(persistentVolumeMetricFamilies(c.AllowAnnotationsList, c.AllowLabelsList, "annotation", "label"))
		c.Headers = generator.ExtractMetricFamilyHeaders(persistentVolumeMetricFamilies(c.AllowAnnotationsList, c.AllowLabelsList, "annotation", "label"))
