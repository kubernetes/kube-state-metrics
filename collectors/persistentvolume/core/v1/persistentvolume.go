...
func RegisterPersistentVolumeCollector(registry prometheus.Registerer, kubeClient kubernetes.Interface, namespace string) {
	...
	pvinf := cache.NewSharedInformer(pvlw, &v1.PersistentVolume{}, collectors.ResyncPeriod)
...
func (collector *persistentVolumeCollector) Collect(ch chan<- prometheus.Metric) {
	...
	if err != nil {
		collectors.ScrapeErrorTotalMetric.With(prometheus.Labels{"resource": "persistentvolume"}).Inc()
		glog.Errorf("listing persistentVolume failed: %s", err)
		return
	}

	collectors.ResourcesPerScrapeMetric.With(prometheus.Labels{"resource": "persistentvolume"}).Observe(float64(len(persistentVolumeCollector.Items)))
...
func (collector *persistentVolumeCollector) collectPersistentVolume(ch chan<- prometheus.Metric, pv v1.PersistentVolume) {
	...
	if p := pv.Status.Phase; p != "" {
		addGauge(descPersistentVolumeStatusPhase, collectors.BoolFloat64(p == v1.VolumePending), string(v1.VolumePending))
		addGauge(descPersistentVolumeStatusPhase, collectors.BoolFloat64(p == v1.VolumeAvailable), string(v1.VolumeAvailable))
		addGauge(descPersistentVolumeStatusPhase, collectors.BoolFloat64(p == v1.VolumeBound), string(v1.VolumeBound))
		addGauge(descPersistentVolumeStatusPhase, collectors.BoolFloat64(p == v1.VolumeReleased), string(v1.VolumeReleased))
		addGauge(descPersistentVolumeStatusPhase, collectors.BoolFloat64(p == v1.VolumeFailed), string(v1.VolumeFailed))
	}
}
