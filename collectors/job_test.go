...
func TestJobCollector(t *testing.T) {
	...
	cases := []struct {
	...
	}{
		{
			...
			want: metadata + `
				kube_job_created{job="FailedJob1",namespace="ns1"} -6.21355968e+10
				kube_job_created{job="RunningJob1",namespace="ns1"} -6.21355968e+10
				kube_job_created{job="SuccessfulJob1",namespace="ns1"} -6.21355968e+10
				kube_job_created{job="SuccessfulJob2NoActiveDeadlineSeconds",namespace="ns1"} -6.21355968e+10
