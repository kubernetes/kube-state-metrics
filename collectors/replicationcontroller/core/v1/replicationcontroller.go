...
func RegisterReplicationControllerCollector(registry prometheus.Registerer, kubeClient kubernetes.Interface, namespace string) {
	...
	rcinf := cache.NewSharedInformer(rclw, &v1.ReplicationController{}, collectors.ResyncPeriod)
...
func (dc *replicationcontrollerCollector) Collect(ch chan<- prometheus.Metric) {
	...
	if err != nil {
		collectors.ScrapeErrorTotalMetric.With(prometheus.Labels{"resource": "replicationcontroller"}).Inc()
		glog.Errorf("listing replicationcontrollers failed: %s", err)
		return
	}

	collectors.ResourcesPerScrapeMetric.With(prometheus.Labels{"resource": "replicationcontroller"}).Observe(float64(len(rcs)))
	for _, d := range rcs {
