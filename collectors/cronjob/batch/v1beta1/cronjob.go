...
func RegisterCronJobCollector(registry prometheus.Registerer, kubeClient kubernetes.Interface, namespace string) {
	...
	cjinf := cache.NewSharedInformer(cjlw, &batchv1beta1.CronJob{}, collectors.ResyncPeriod)
...
func (cjc *cronJobCollector) Collect(ch chan<- prometheus.Metric) {
	...
	if err != nil {
		collectors.ScrapeErrorTotalMetric.With(prometheus.Labels{"resource": "cronjob"}).Inc()
		glog.Errorf("listing cronjobs failed: %s", err)
		return
	}

	collectors.ResourcesPerScrapeMetric.With(prometheus.Labels{"resource": "cronjob"}).Observe(float64(len(cronjobs)))
...
func (jc *cronJobCollector) collectCronJob(ch chan<- prometheus.Metric, j batchv1beta1.CronJob) {
	...
	if j.Spec.Suspend != nil {
		addGauge(descCronJobSpecSuspend, collectors.BoolFloat64(*j.Spec.Suspend))
	}
