...
func RegisterResourceQuotaCollector(registry prometheus.Registerer, kubeClient kubernetes.Interface, namespace string) {
	...
	rqinf := cache.NewSharedInformer(rqlw, &v1.ResourceQuota{}, collectors.ResyncPeriod)
...
func (rqc *resourceQuotaCollector) Collect(ch chan<- prometheus.Metric) {
	...
	if err != nil {
		collectors.ScrapeErrorTotalMetric.With(prometheus.Labels{"resource": "resourcequota"}).Inc()
		glog.Errorf("listing resource quotas failed: %s", err)
		return
	}

	collectors.ResourcesPerScrapeMetric.With(prometheus.Labels{"resource": "resourcequota"}).Observe(float64(len(resourceQuota.Items)))
