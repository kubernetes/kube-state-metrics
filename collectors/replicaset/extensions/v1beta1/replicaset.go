...
func RegisterReplicaSetCollector(registry prometheus.Registerer, kubeClient kubernetes.Interface, namespace string) {
	...
	rsinf := cache.NewSharedInformer(rslw, &v1beta1.ReplicaSet{}, collectors.ResyncPeriod)
...
func (dc *replicasetCollector) Collect(ch chan<- prometheus.Metric) {
	...
	if err != nil {
		collectors.ScrapeErrorTotalMetric.With(prometheus.Labels{"resource": "replicaset"}).Inc()
		glog.Errorf("listing replicasets failed: %s", err)
		return
	}

	collectors.ResourcesPerScrapeMetric.With(prometheus.Labels{"resource": "replicaset"}).Observe(float64(len(rss)))
	for _, d := range rss {
