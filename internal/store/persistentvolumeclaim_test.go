...
`,
			MetricNames: []string{"kube_persistentvolumeclaim_info", "kube_persistentvolumeclaim_status_phase", "kube_persistentvolumeclaim_resource_requests_storage_bytes", "kube_persistentvolumeclaim_access_mode", "kube_persistentvolumeclaim_status_condition"},
		},
		{
			Obj: &v1.PersistentVolumeClaim{
				ObjectMeta: metav1.ObjectMeta{
		...
		},
		{
			...
			Want: `
				...
				kube_persistentvolumeclaim_annotations{annotation_allowlisted="true",namespace="",persistentvolumeclaim="mongo-data"} 1
`,
	...
	for i, c := range cases {
		c.Func = generator.ComposeMetricGenFuncs(persistentVolumeClaimMetricFamilies(c.AllowLabelsList, c.AllowAnnotationsList))
		c.Headers = generator.ExtractMetricFamilyHeaders(persistentVolumeClaimMetricFamilies(c.AllowLabelsList, c.AllowAnnotationsList))
