...
func RegisterPodCollector(registry prometheus.Registerer, kubeClient kubernetes.Interface, namespace string) {
	...
	pinf := cache.NewSharedInformer(plw, &v1.Pod{}, collectors.ResyncPeriod)
...
func (pc *podCollector) Collect(ch chan<- prometheus.Metric) {
	...
	if err != nil {
		collectors.ScrapeErrorTotalMetric.With(prometheus.Labels{"resource": "pod"}).Inc()
		glog.Errorf("listing pods failed: %s", err)
		return
	}

	collectors.ResourcesPerScrapeMetric.With(prometheus.Labels{"resource": "pod"}).Observe(float64(len(pods)))
	for _, p := range pods {
		pc.collectPod(ch, p)
	}
...
func (pc *podCollector) collectPod(ch chan<- prometheus.Metric, p v1.Pod) {
	...
	labelKeys, labelValues := collectors.KubeLabelsToPrometheusLabels(p.Labels)
	addGauge(podLabelsDesc(labelKeys), 1, labelValues...)

	if p := p.Status.Phase; p != "" {
		addGauge(descPodStatusPhase, collectors.BoolFloat64(p == v1.PodPending), string(v1.PodPending))
		addGauge(descPodStatusPhase, collectors.BoolFloat64(p == v1.PodRunning), string(v1.PodRunning))
		addGauge(descPodStatusPhase, collectors.BoolFloat64(p == v1.PodSucceeded), string(v1.PodSucceeded))
		addGauge(descPodStatusPhase, collectors.BoolFloat64(p == v1.PodFailed), string(v1.PodFailed))
		addGauge(descPodStatusPhase, collectors.BoolFloat64(p == v1.PodUnknown), string(v1.PodUnknown))
	}
	...
	for _, c := range p.Status.Conditions {
		switch c.Type {
		case v1.PodReady:
			collectors.AddConditionMetrics(ch, descPodStatusReady, c.Status, p.Namespace, p.Name)
		case v1.PodScheduled:
			collectors.AddConditionMetrics(ch, descPodStatusScheduled, c.Status, p.Namespace, p.Name)
		}
	}
	...
	for _, cs := range p.Status.ContainerStatuses {
		...
		addGauge(descPodContainerStatusWaiting, collectors.BoolFloat64(cs.State.Waiting != nil), cs.Name)
		for _, reason := range containerWaitingReasons {
			addGauge(descPodContainerStatusWaitingReason, collectors.BoolFloat64(waitingReason(cs, reason)), cs.Name, reason)
		}
		addGauge(descPodContainerStatusRunning, collectors.BoolFloat64(cs.State.Running != nil), cs.Name)
		addGauge(descPodContainerStatusTerminated, collectors.BoolFloat64(cs.State.Terminated != nil), cs.Name)
		for _, reason := range containerTerminatedReasons {
			addGauge(descPodContainerStatusTerminatedReason, collectors.BoolFloat64(terminationReason(cs, reason)), cs.Name, reason)
		}
		addGauge(descPodContainerStatusReady, collectors.BoolFloat64(cs.Ready), cs.Name)
