...
func RegisterEndpointCollector(registry prometheus.Registerer, kubeClient kubernetes.Interface, namespace string) {
	...
	sinf := cache.NewSharedInformer(slw, &v1.Endpoints{}, collectors.ResyncPeriod)
...
func (ec *endpointCollector) Collect(ch chan<- prometheus.Metric) {
	...
	if err != nil {
		collectors.ScrapeErrorTotalMetric.With(prometheus.Labels{"resource": "endpoint"}).Inc()
		glog.Errorf("listing endpoints failed: %s", err)
		return
	}

	collectors.ResourcesPerScrapeMetric.With(prometheus.Labels{"resource": "endpoint"}).Observe(float64(len(endpoints)))
...
func (ec *endpointCollector) collectEndpoints(ch chan<- prometheus.Metric, e v1.Endpoints) {
	...
	labelKeys, labelValues := collectors.KubeLabelsToPrometheusLabels(e.Labels)
