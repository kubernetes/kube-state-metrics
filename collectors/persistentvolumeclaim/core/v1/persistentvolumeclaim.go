...
func RegisterPersistentVolumeClaimCollector(registry prometheus.Registerer, kubeClient kubernetes.Interface, namespace string) {
	...
	pvcinf := cache.NewSharedInformer(pvclw, &v1.PersistentVolumeClaim{}, collectors.ResyncPeriod)
...
func (collector *persistentVolumeClaimCollector) Collect(ch chan<- prometheus.Metric) {
	...
	if err != nil {
		collectors.ScrapeErrorTotalMetric.With(prometheus.Labels{"resource": "persistentvolumeclaim"}).Inc()
		glog.Errorf("listing persistent volume claims failed: %s", err)
		return
	}

	collectors.ResourcesPerScrapeMetric.With(prometheus.Labels{"resource": "persistentvolumeclaim"}).Observe(float64(len(persistentVolumeClaimCollector.Items)))
...
func (collector *persistentVolumeClaimCollector) collectPersistentVolumeClaim(ch chan<- prometheus.Metric, pvc v1.PersistentVolumeClaim) {
	...
	if p := pvc.Status.Phase; p != "" {
		addGauge(descPersistentVolumeClaimStatusPhase, collectors.BoolFloat64(p == v1.ClaimLost), string(v1.ClaimLost))
		addGauge(descPersistentVolumeClaimStatusPhase, collectors.BoolFloat64(p == v1.ClaimBound), string(v1.ClaimBound))
		addGauge(descPersistentVolumeClaimStatusPhase, collectors.BoolFloat64(p == v1.ClaimPending), string(v1.ClaimPending))
	}
