...
func RegisterServiceCollector(registry prometheus.Registerer, kubeClient kubernetes.Interface, namespace string) {
	...
	sinf := cache.NewSharedInformer(slw, &v1.Service{}, collectors.ResyncPeriod)
...
func (sc *serviceCollector) Collect(ch chan<- prometheus.Metric) {
	...
	if err != nil {
		collectors.ScrapeErrorTotalMetric.With(prometheus.Labels{"resource": "service"}).Inc()
		glog.Errorf("listing services failed: %s", err)
		return
	}

	collectors.ResourcesPerScrapeMetric.With(prometheus.Labels{"resource": "service"}).Observe(float64(len(services)))
...
func (sc *serviceCollector) collectService(ch chan<- prometheus.Metric, s v1.Service) {
	...
	labelKeys, labelValues := collectors.KubeLabelsToPrometheusLabels(s.Labels)
