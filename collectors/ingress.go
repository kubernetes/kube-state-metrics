...
func RegisterIngressCollector(registry prometheus.Registerer, kubeClient kubernetes.Interface, namespace string) {
	...
	rslw := cache.NewListWatchFromClient(client, "ingresses", namespace, fields.Everything())
...
func (ic *ingressCollector) Collect(ch chan<- prometheus.Metric) {
	icl, err := ic.store.List()
	if err != nil {
		glog.Errorf("listing ingresses failed: %s", err)
		return
	}
	for _, d := range icl.Items {
		ic.collectIngress(ch, d)
	}
	glog.Infof("collected %d ingress", len(icl.Items))
}
