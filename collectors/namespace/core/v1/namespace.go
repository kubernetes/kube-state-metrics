...
func RegisterNamespaceCollector(registry prometheus.Registerer, kubeClient kubernetes.Interface, namespace string) {
	...
	nsinf := cache.NewSharedInformer(nslw, &v1.Namespace{}, collectors.ResyncPeriod)
...
func (nsc *namespaceCollector) Collect(ch chan<- prometheus.Metric) {
	...
	if err != nil {
		collectors.ScrapeErrorTotalMetric.With(prometheus.Labels{"resource": "namespace"}).Inc()
		glog.Errorf("listing namespace failed: %s", err)
		return
	}

	collectors.ResourcesPerScrapeMetric.With(prometheus.Labels{"resource": "namespace"}).Observe(float64(len(nsls)))
	for _, rq := range nsls {
...
func (nsc *namespaceCollector) collectNamespace(ch chan<- prometheus.Metric, ns v1.Namespace) {
	...
	addGauge(descNamespacePhase, collectors.BoolFloat64(ns.Status.Phase == v1.NamespaceActive), string(v1.NamespaceActive))
	addGauge(descNamespacePhase, collectors.BoolFloat64(ns.Status.Phase == v1.NamespaceTerminating), string(v1.NamespaceTerminating))

	if !ns.CreationTimestamp.IsZero() {
		addGauge(descNamespaceCreated, float64(ns.CreationTimestamp.Unix()))
	}

	labelKeys, labelValues := collectors.KubeLabelsToPrometheusLabels(ns.Labels)
