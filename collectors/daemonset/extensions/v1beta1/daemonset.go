...
func RegisterDaemonSetCollector(registry prometheus.Registerer, kubeClient kubernetes.Interface, namespace string) {
	...
	dsinf := cache.NewSharedInformer(dslw, &v1beta1.DaemonSet{}, collectors.ResyncPeriod)
...
func (dc *daemonsetCollector) Collect(ch chan<- prometheus.Metric) {
	...
	if err != nil {
		collectors.ScrapeErrorTotalMetric.With(prometheus.Labels{"resource": "daemonset"}).Inc()
		glog.Errorf("listing daemonsets failed: %s", err)
		return
	}

	collectors.ResourcesPerScrapeMetric.With(prometheus.Labels{"resource": "daemonset"}).Observe(float64(len(dss)))
	for _, d := range dss {
