...
func RegisterLimitRangeCollector(registry prometheus.Registerer, kubeClient kubernetes.Interface, namespace string) {
	...
	rqinf := cache.NewSharedInformer(rqlw, &v1.LimitRange{}, collectors.ResyncPeriod)
...
func (lrc *limitRangeCollector) Collect(ch chan<- prometheus.Metric) {
	...
	if err != nil {
		collectors.ScrapeErrorTotalMetric.With(prometheus.Labels{"resource": "limitrange"}).Inc()
		glog.Errorf("listing limit ranges failed: %s", err)
		return
	}

	collectors.ResourcesPerScrapeMetric.With(prometheus.Labels{"resource": "limitrange"}).Observe(float64(len(limitRangeCollector.Items)))
