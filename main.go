...
type collectors map[string]struct{}
...
func (c *collectors) Set(value string) error {
	s := *c
...
func (c collectors) asSlice() []string {
	cols := []string{}
	for col, _ := range c {
...
func (c collectors) isEmpty() bool {
	return len(c.asSlice()) == 0
}

func (c *collectors) Type() string {
...
func main() {
	...
	options := &options{collectors: make(collectors)}
	...
	if len(options.collectors) == 0 {
		...
		glog.Infof("Reqested collectors: %s", options.collectors.String())
...
func registerCollectors(kubeClient clientset.Interface, enabledCollectors collectors) {
	...
	for c, _ := range enabledCollectors {
		...
		if !ok {
			glog.Warningf("Collector \"%s\" does not exist", c)
		}
	}
